...
if (isset($_GET['type']) && $_GET['type'] == 'update' && isset($_GET['id'])) {
   ...
   if ($status == '2') {
        ...
        mysqli_query($con, "update leave_applied set HODLS = '1' where LID ='" . $id . "'");
...
$res = mysqli_query($con, "select * from leave_applied where FAID = '" . $_SESSION['USER_ID'] . "'");
...
<script>
    function update_status(lid, value) {
        window.location.href = "http://localhost/lms/Faculty/leave_request.php?id=" + lid + "&type=update&status=" + value;
    }
</script>
<?php
