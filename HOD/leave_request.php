...
if (isset($_GET['type']) && $_GET['type'] == 'update' && isset($_GET['id'])) {
   ...
   if ($status == '2') {
        mysqli_query($con, "update leave_applied set HODLS = '2' where LID ='" . $id . "'");
        mysqli_query($con, "update leave_applied set PLS = '1' where LID ='" . $id . "'");
    } else  if ($status == '3') {
        mysqli_query($con, "update leave_applied set HODLS = '3' where LID ='" . $id . "'");
...
$res = mysqli_query($con, "select * from leave_applied where DID = '" . $_SESSION['DID'] . "' and Leave_status = '2'");
...
<script>
    function update_status(lid, value) {
        window.location.href = "http://localhost/lms/HOD/leave_request.php?id=" + lid + "&type=update&status=" + value;
    }
</script>
<?php
